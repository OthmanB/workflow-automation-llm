# OpenCode 1.18.18 Compatibility Fixtures

These sanitized fixtures define the external CLI shapes that the dispatcher
must support before real OpenCode execution is enabled.

## Provenance

- Installed version verified with `opencode --version`: `1.18.18`.
...
- Sanitized export structure is derived from:
  `https://github.com/anomalyco/opencode/blob/v1.18.18/packages/opencode/src/cli/cmd/export.ts`.
- Import output is derived from:
  `https://github.com/anomalyco/opencode/blob/v1.18.18/packages/opencode/src/cli/cmd/import.ts`.
- Session-list structure is derived from the exact tagged
  `packages/opencode/src/cli/cmd/session.ts` source and verified locally with
  `opencode session list -n 1 --format json`.
- The local 1.18.18 help confirms that export/import are top-level commands:
...
2. Review the matching tagged `run.ts`, `export.ts`, `import.ts`, and session-list source.
3. Refresh all affected fixtures without using real prompts or IDs.
4. Run the fixture sanitation and decoder contract tests.
5. Update `[tool.dispatcher.opencode]` in `pyproject.toml` in the same change.
6. Do not enable real dispatch until the live harmless compatibility smoke test
